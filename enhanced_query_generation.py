...
## enhanced_query_generation.py is
## uses viterbi algorithm to tag each word in a question as either a key word  (denoted by the tag K )or a non keyword
...
question = raw_input()
related_corpus = "1_related_corpus.txt"
unrelated_corpus = "1_unrelated_corpus.txt"

##Preprocessing the question
question = preprocess(question)
list_of_words_in_question = question.split()
##print "For the observation sequence equal to he words in  "
##pprint( list_of_words_in_question )

#Computation of emission probabilites for the Hidden Markov Model
e, list_of_words_related_corpus, list_of_words_unrelated_corpus = obtain_emission_probailities(list_of_words_in_question, related_corpus, unrelated_corpus)

##Query generation pipeline:

#2. Decoding using Viterbi algorithm
tags =  viterbi(list_of_words_in_question,q,e,initial_state_probabilities) 

query = [list_of_words_in_question[idx] for idx in range(len(tags)) if tags[idx] == 'K']
##print "Query: "
##pprint(query)

#3. Refining the query bby using Part of Speech tagging
queryWords_and_tags = pos_tag((word_tokenize(" ".join(query))))
##print "Tagged query: "
##pprint(queryWords_and_tags)

refined_query = []
for w,t in queryWords_and_tags:
    if t == 'JJ' and e[(w,'K')] > e[(w,'NK')]:
        refined_query.append(w)
...
#4. Returning the query if it is relevant to the corpus
if is_the_query_relevent_LESK(list_of_words_related_corpus, list_of_words_unrelated_corpus, set(refined_query) ):
    pprint(refined_query)
else:
    print []
